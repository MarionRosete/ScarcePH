const items = [
    { 
        image: "./public/og2012.webp", 
        name: 'Nike Sb Stefan Janoski OG Venom 2012',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/hRUCD5FJGi8MhCTa/'
    },
   { 
        image: "./public/og2018.png", 
        name: 'Nike Sb Stefan Janoski OG Venom 2018',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/ThaXUckGb4cDjNxY/'
    },
    { 
        image: "./public/vastgray.png", 
        name: 'Nike Sb Stefan Janoski Vast Gray',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/vbDqnguXCk3HctQJ/'
    },
    { 
        image: "./public/slip+white.png", 
        name: 'Nike Sb Stefan Janoski Slip+ white',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/dPu6avKktYL8WSYH/'
    },
    { 
        image: "./public/flamingo.webp", 
        name: 'Nike Sb Stefan Janoski Flamingo ',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/ujxhLnYL1QgcZjkG/'
    },
   { 
       image: "./public/blackfloral.webp", 
        name: 'Nike Sb Stefan Janoski Black Floral',
        size: ['10us'],
        url: 'https://www.facebook.com/share/p/hyusgfjK3NKAWYe4/'
    },

     { 
        image: "./public/woodgrain.webp", 
        name: 'Nike Sb Stefan Janoski Wood Grain',
        size: ['10us'],
        url:'https://www.facebook.com/share/p/seeJWQchQ8WFkAxH/'
    },
    { 
        image: "./public/rmcanvas.png", 
        name: 'Nike Sb Stefan Janoski RM Canvas',
        size: ['10us'],
        url: 'https://www.facebook.com/share/p/iQipxbWPt5Mt9KuY/'
    },
     { 
       image: "./public/urbancamo.webp", 
        name: 'Nike Sb Stefan Janoski Urban Camo',
        size: ['9.5us'],
        url: 'https://www.facebook.com/share/p/N4bjFSRueMWzStL9/'
    },
    { 
       image: "./public/britishtan.webp", 
        name: 'Nike Sb Stefan Janoski British Tan',
        size: ['8.5us'],
        url: 'https://www.facebook.com/share/p/Z9DMgNMF7irzpj3d/'
    },
     { 
       image: "./public/slip+black.webp", 
        name: 'Nike Sb Stefan Janoski Slip+ Venom',
        size: ['8.5us'],
        url: 'https://www.facebook.com/share/p/Yr2Z1yb2kuzG4jcp/'
    },
     { 
       image: "./public/aztecblack.webp", 
        name: 'Nike Sb Stefan Janoski RM Black Aztec',
        size: ['8us'],
        url:'https://www.facebook.com/share/p/CwpsGmPZCJdHKzU9/'
    },
    { 
        image: "./public/phantomblue.png", 
        name: 'Nike Sb Stefan Janoski Obsidian Phantom Blue',
        size: ['8us'],
        url: 'https://www.facebook.com/share/p/zaNyRw9TtKEEsZfD/'
    },
    { 
        image: "./public/midnight.webp", 
        name: 'Nike Sb Stefan Janoski Midnight Green',
        size: ['8us'],
        url: 'https://www.facebook.com/share/p/6Kdf27RuAPxhd3US/'
    },
    { 
       image: "./public/irongray.webp", 
        name: 'Nike Sb Stefan Janoski RM Iron Gray',
        size: ['7.5us'],
        url: 'https://www.facebook.com/share/p/Yekk3qM3vpQ5AR9b/'
    },
    { 
       image: "./public/chron2.webp", 
        name: 'Nike Sb Chron 2',
        size: ['10.5us'],
        url:'https://www.facebook.com/share/p/7ZE8tMdPgkXW8Ld6/'
    },
    { 
       image: "./public/force58.png", 
        name: 'Nike Sb Force 58',
        size: ['9us'],
        url:'https://www.facebook.com/Scarceph/posts/pfbid08Qdwt8yBRJhdzg8FyLv92kpM1X87bMm1YDR2N3YFH8wSEqfWFxTDo2y1SQFLVAHzl'
    },
    { 
        image: "./public/af1.png", 
        name: 'Nike Air Force 1 FlyEase',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/dg8mYxXqL52J6jx4/'
    },
    { 
        image: "./public/vanslip.png", 
        name: 'Vans Classic Slip',
        size: ['8us','10us'],
        url: 'https://www.facebook.com/share/p/pJhH3rmWALWLWXZm/'
    },
    { 
        image: "./public/dcslip.png", 
        name: 'DC Suede slip',
        size: ['10.5us'],
        url: 'https://www.facebook.com/share/p/DjSiifPPvXvgtzbz/'
    },
   
];

export default items; 
